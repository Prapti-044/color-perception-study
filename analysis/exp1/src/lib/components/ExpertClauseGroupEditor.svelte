<script lang="ts">
	...
	function replaceChild(index: number, child: ExpertClauseNode) {
		replaceChildren(node.children.map((currentChild, childIndex) => (
...
<div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm" style={`margin-left: ${depth * 0.75}rem;`}>
		...
		<div class="flex flex-wrap items-center gap-2">
			...
			{#if !isRoot && onRemove}
				<button
					type="button"
					onclick={onRemove}
					class="rounded border border-red-200 px-3 py-1 text-xs font-medium text-red-700 hover:bg-red-50"
				>
					Remove Group
				</button>
			{/if}
	...
	<div class="mt-4 space-y-3">
		{#if node.children.length === 0}
			<div class="rounded-md border border-dashed border-slate-300 bg-slate-50 px-3 py-4 text-sm italic text-slate-500">
		...
		{#each node.children as child, index (child.id)}
			{#if child.type === 'group'}
				...
				/>
			{:else}
				<div class="flex flex-wrap items-center justify-between gap-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-3">
	...
	<div class="mt-4 flex flex-wrap items-center gap-2">
		<button
			type="button"
			onclick={() => showPredicatePalette = !showPredicatePalette}
			class="rounded border border-blue-200 px-3 py-2 text-sm font-medium text-blue-700 hover:bg-blue-50"
		>
		...
		<button
			type="button"
			onclick={addSubgroup}
			class="rounded border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
		>
			Add Nested Group
		</button>
	</div>
	...
	{#if showPredicatePalette}
		<div class="mt-4 space-y-4 rounded-lg border border-blue-100 bg-blue-50 p-4">
					...
					<div class="flex flex-wrap gap-2">
						{#each predicates as predicate (predicate.id)}
							<button
								...
								class="rounded border border-blue-200 bg-white px-3 py-2 text-left text-xs text-slate-700 hover:bg-blue-100"
							>
								{predicate.label}
							</button>
						{/each}
