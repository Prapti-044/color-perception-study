// Constants for the Colormap Makeup Study Analysis
...
export const ATTENTION_CHECK_QUESTIONS: Record<
	number,
...
// Original study: 72 participants (24 per axis), 6 sizes × 6 ΔE levels × 2 reps = 72 trials/participant
export const ORIGINAL_PAPER_RESULTS = {
	regression: {
		L: {
	...
	} as Record<string, Record<number, { slope: number; r2: number; nd50: number }>>,
	// Inverse-size model: ND_x(p,s) = p / (c_x + k_x/s)
	// Note: k values are negative in the paper's formulation (subtraction)
	inverse_model: {
		L: { c: 0.0937, k: -0.0085, r2: 0.9 },
		a: { c: 0.0775, k: -0.0121, r2: 0.97 },
		b: { c: 0.0611, k: -0.0096, r2: 0.9 }
	} as Record<string, { c: number; k: number; r2: number }>,
	study_params: {
		n_per_axis: 24,
		n_total: 72,
